...
def evaluate_sparse_retreival(
    ...
    **kwargs,
    ...
    if accelerator.is_local_main_process:
        ...
        # search
        search_result_filename = f"{eval_dataset}_search_result_hits{hits}.trec"
        ...
        org_marco_dev_qrels_path = os.path.join(
            args.data_dir, f"passage_ranking/qrels.{eval_dataset}.tsv")
...
class AnseriniInterface(object):
    @staticmethod
    def do_index(anserini_path, saved_index_dir, saved_doc_emb_dir, num_index_splits):
        logging.info("indexing via anserini ...")
        os.system(
            f"sh {anserini_path}/target/appassembler/bin/IndexCollection -collection JsonVectorCollection \
             -input {saved_doc_emb_dir} \
    ...
    @staticmethod
    def do_search(anserini_path, search_result_path, saved_index_dir, saved_query_emb_path, hits):
        logging.info("Doing search ...")
        os.system(
            f"sh {anserini_path}/target/appassembler/bin/SearchCollection -hits {hits} -parallelism 80 \
             -index {saved_index_dir} \
